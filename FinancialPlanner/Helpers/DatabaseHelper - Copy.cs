...
namespace FinancialPlanner.Helpers
{
    public static class DatabaseHelper
    {
        ...
        public static void GenerateTransactions(int accountId)
        {
            ...
            List<Budget> budgets = db.Budgets.Where(b => b.HouseholdId == account.HouseholdId).ToList();
            ...
            for (int n = 30; n >= 0; n--)
            {
                if (DateTime.Now.AddDays(-n).Month == DateTime.Now.Month)
                {
                    ...
                    else
                    {
                        ...
                        int budgetChoice = rnd.Next(1, BudgetItemsDictionary.Count() + 1);
                        List<int> budgetItemList = new List<int>();
                        BudgetItemsDictionary.TryGetValue(db.Budgets.Find(budgetChoice), out budgetItemList);
